...
app.get('/products',(req, res)=>{           // "/products?limite=nro"
    ...
    if(!limite || !(limite > 0)) return res.send({productosIn});
    res.send(productosIn.slice(0,limite));
})
...
app.get('/products/:pid',(req, res)=>{
    ...
    let product = productosIn.find(p=>p.id===idProduct);
    if(!product) return res.send({error: "Producto no encontrado"});
    res.send({product});
})
